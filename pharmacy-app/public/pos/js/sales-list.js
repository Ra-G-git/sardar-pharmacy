          ...
          <tr class="sales-row" data-id="${o.id}">
            ...
            <td class="text-center">
              <div style="display:flex; justify-content:center; gap:6px;">
                ...
                ${isAdmin ? `
                  <button class="btn btn-secondary btn-sm btn-delete-invoice" title="Delete Order" style="color:var(--danger);">🗑️</button>
                ` : ''}
              </div>
            </td>
          </tr>
        ...
        if (isAdmin) {
          ...
          if (deleteBtn) {
            deleteBtn.onclick = async () => {
              if (await H.confirm(`⚠️ WARNING: Deleting this order will RESTORE product/variation inventory levels and permanently delete all transaction data.\n\nAre you sure you want to delete order ${order.invoiceId}?`)) {
                deleteBtn.disabled = true;
                const success = await S.delete('orders', order.id);
                if (success) {
                  H.showToast(`Order ${order.invoiceId} deleted successfully.`);
                  await this.updateList(from, to, search);
                } else {
