(function () {
  ...
  const LabelPrinter = {
    ...
    async render() {
                ...
                <div style="display:flex; gap:10px;">
                  ...
                  <div class="form-group" style="flex:1; margin-bottom:0;">
                    <select class="form-select" id="label-cat-select" style="height:38px;">
                      <option value="">-- Add Entire Generic --</option>
                    ...
                    <tbody id="selected-prods-body">
                      <tr>
                        <td colspan="3" class="text-center text-muted" style="padding:20px;">
                          No products selected. Search above or select a generic name to add products.
                        </td>
                      </tr>
                    </tbody>
                  </table>
      ...
      // Generic selector trigger
      const catSelect = document.getElementById('label-cat-select');
      catSelect.onchange = async () => {
        const genVal = catSelect.value;
        if (!genVal) return;

        const catProds = products.filter(p => p.generic === genVal);
        if (catProds.length === 0) {
          H.showToast('No products with this generic name.', 'warning');
        } else {
