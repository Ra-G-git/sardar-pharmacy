    ...
    async render(orderId) {
      ...
      this.cart = items.map(i => {
        ...
        if (p && i.variationName && p.variations) {
          ...
          if (v) {
            const qtyPerUnit = v.qty_per_unit || 1;
            const totalPieces = p.stock + (i.qty * qtyPerUnit);
            stock = Math.floor(totalPieces / qtyPerUnit);
          }
        } else {
          if (p) stock = p.stock + i.qty;
        }
        return {
          ...
          if (mainMatch) {
            if (p.variations && p.variations.length > 0) {
              p.variations.forEach(v => {
                ...
                matches.push({ product: p, variant: v, name: `${p.name || ''} (${v.name || ''})`, sku: v.sku, barcode: v.barcode, price: calculatedPrice, stock: calculatedStock });
              });
            } else {
          ...
          for (const p of prods) {
            if (p.variations && p.variations.length > 0) {
              ...
              if (v) {
                const calculatedStock = Math.floor(p.stock / (v.qty_per_unit || 1));
                const calculatedPrice = v.price || (p.sellingPrice * (v.qty_per_unit || 1));
                matched = { product: p, variant: v, name: `${p.name} (${v.name})`, sku: v.sku, barcode: v.barcode, price: calculatedPrice, stock: calculatedStock };
                break;
              }
            } else {
