(function () {
  ...
  const App = {
    init() {
      ...
      R.register('/gallery',         () => POS.Gallery.render());
      R.register('/inventory-details', () => POS.InventoryDetails.render());
      ...
      R.register('/prescriptions',   () => POS.Prescriptions.render());
      R.register('/online-orders',   () => POS.OnlineOrders.render());
      R.register('/site-users',      () => POS.SiteUsers.render());
    ...
    _setupSidebar() {
      ...
      if (user) {
        ...
        if (role === 'cashier') {
          // Hide Dashboard, Reports, Users, Coupons, Settings
          ...
          // Show the Catalog container itself but hide restricted sub-pages
          const catalogGrp = document.getElementById('nav-grp-catalog');
          if (catalogGrp) {
            catalogGrp.style.display = '';
            catalogGrp.querySelectorAll('.nav-item').forEach(el => {
              const page = el.getAttribute('data-page');
              if (page === 'inventory-details') {
                el.style.display = '';
              } else {
          ...
          // Show the separate Inventory container if present
          const invGrp = document.getElementById('nav-grp-inventory');
          if (invGrp) invGrp.style.display = '';
          ...
          // Show all items inside Catalog container
          const catalogGrp = document.getElementById('nav-grp-catalog');
          if (catalogGrp) {
            catalogGrp.style.display = '';
            catalogGrp.querySelectorAll('.nav-item').forEach(el => {
