(function () {
  ...
  const Reports = {
    async render(type) {
      ...
      // Always default to all-time — not persisted across visits, see
      // the same fix/reasoning in dashboard.js.
      let fromDate = '';
      let toDate = '';
    ...
    dailySaleReport(container, orders, H) {
      ...
      orders.forEach(o => {
        ...
        salesByDay[day].subtotal += parseFloat(o.subtotal) || 0;
        salesByDay[day].discount += parseFloat(o.discountAmount) || 0;
        salesByDay[day].tax += parseFloat(o.taxAmount) || 0;
        salesByDay[day].total += parseFloat(o.grandTotal) || 0;
      });
          ...
          p.variations.forEach(v => {
            ...
            tbodyRows += `
              <tr>
                ...
                <td>${H.esc(brandName)}</td>
                <td>${H.esc(genericName)}</td>
          ...
          <div class="card-body">
            <div class="table-wrapper">
              <table class="data-table">
                ...
                <tbody>
                  ${tbodyRows || '<tr><td colspan="9" class="text-center text-muted">No stock available.</td></tr>'}
                  ${tbodyRows ? `
                    <tr class="total-row">
                      <td colspan="4">Stock Summary Valuations</td>
    ...
    salesSummaryReport(container, orders, H) {
      ...
      // parseFloat(...) || 0 guards against older order records missing a
      // field entirely — a single undefined would otherwise turn the whole
      // sum into NaN (displayed as ৳0.00 for every total, not just that field).
      let totalSales = orders.reduce((s, o) => s + (parseFloat(o.grandTotal) || 0), 0);
      let totalTax = orders.reduce((s, o) => s + (parseFloat(o.taxAmount) || 0), 0);
      let totalDiscount = orders.reduce((s, o) => s + (parseFloat(o.discountAmount) || 0), 0);
      let totalPaid = orders.reduce((s, o) => s + (parseFloat(o.paidAmount) || 0), 0);
      let totalDue = orders.reduce((s, o) => s + (parseFloat(o.dueAmount) || 0), 0);
      let totalReturned = orders.reduce((s, o) => s + (parseFloat(o.returnedAmount) || 0), 0);
