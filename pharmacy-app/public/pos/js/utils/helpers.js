(function () {
  ...
  const Helpers = {
    ...
    formatDateTime(dateStr) {
      if (!dateStr) return '';
      // Firestore Timestamps come back from the API as {_seconds, _nanoseconds}
      // (or {seconds} from some SDK paths) rather than a plain date string.
      let d;
      if (typeof dateStr === 'object' && (dateStr._seconds !== undefined || dateStr.seconds !== undefined)) {
        d = new Date((dateStr._seconds ?? dateStr.seconds) * 1000);
      } else {
        d = new Date(dateStr);
      }
    ...
    toMillis(dateVal) {
      if (!dateVal) return 0;
      if (typeof dateVal === 'object' && (dateVal._seconds !== undefined || dateVal.seconds !== undefined)) {
        return (dateVal._seconds ?? dateVal.seconds) * 1000;
      }
    ...
    async printOrder(rawOrder, rawItems, localPayments = null) {
      const S = POS.Store;
      const settings = await S.getSettings();
      const printType = settings.default_print_type || 'receipt';

      // Normalize field names — a freshly-placed order (from new-order.js,
      // before saving) and a stored order fetched back from Firestore later
      // (from sales-list.js, for reprints) use different field names for the
      // same data. Reading only one shape meant reprints showed blank
      // customer/item info. This accepts either.
      const order = {
        ...rawOrder,
        customerName: rawOrder.customerName || rawOrder.name || 'Walk-in Customer',
        customerPhone: rawOrder.customerPhone || rawOrder.phone || 'N/A',
        grandTotal: rawOrder.grandTotal ?? rawOrder.total ?? 0,
        discountAmount: rawOrder.discountAmount ?? (
          rawOrder.discountType === 'flat'
            ? (rawOrder.flatDiscount || 0)
            : ((rawOrder.subtotal || 0) * (rawOrder.discount || 0) / 100)
        ),
      };
      ...
      // customer-facing site's whatsappReceipt().
      const waItemLines = items.map(it =>
        `\u2022 ${it.productName}${it.variationName ? ` (${it.variationName})` : ''} x${it.qty} = ${this.formatCurrency(it.total)}`
      ).join('\n');
      const waMessage =
`*${storeName || 'Receipt'}*
${storeAddress}
...
Date: ${this.formatDateTime(order.date)}
--------------------------------
...
${waItemLines}
--------------------------------
*TOTAL: ${this.formatCurrency(order.grandTotal)}*
--------------------------------
_Thank you for shopping with us!_`;
      let waPhone = (order.customerPhone || '').trim().replace(/\s+/g, '');
      if (waPhone && waPhone !== 'N/A') {
        if (waPhone.startsWith('0')) waPhone = '880' + waPhone.slice(1);
        else if (!waPhone.startsWith('880')) waPhone = '880' + waPhone;
      } else {
        waPhone = '';
      }
      ...
      const logoUrl = settings.invoice_logo && settings.invoice_logo.startsWith('/')
        ? window.location.origin + settings.invoice_logo
        ...
        printContent = `
          <div class="thermal-receipt ${settings.receipt_style || 'style-1'}">
            ...
            <div style="text-align: center; margin-bottom: 8px;">
              ${settings.invoice_logo ? `<img src="${logoUrl}" style="max-width: 60px; max-height: 60px; object-fit: contain; margin-bottom: 6px; display: block; margin-left: auto; margin-right: auto;">` : ''}
            ...
            <div style="text-align: center; margin-top: 15px; font-size: 8pt;">
              ...
              <img src="${qrCodeUrl}" style="width:70px; height:70px; margin-top:8px;" alt="QR">
            </div>
            <div class="no-print" style="text-align:center; margin-top:16px;">
              <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block; padding:10px 22px; background:#25D366; color:#fff; border-radius:8px; text-decoration:none; font-weight:700; font-size:11px; font-family:'Inter',sans-serif;">
                📱 Share via WhatsApp
              </a>
            </div>
          </div>
          ...
          <div class="paper-invoice ${settings.invoice_style || 'theme-modern'}">
            <div class="invoice-top">
              <div class="store-details" style="display:flex; align-items:center; gap:16px;">
                ${settings.invoice_logo ? `<img src="${logoUrl}" style="max-width: 80px; max-height: 80px; object-fit: contain; border-radius: 4px;">` : ''}
                <div>
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; margin-top:24px; padding-top:16px; border-top:1px dashed #cbd5e1;">
              <img src="${qrCodeUrl}" style="width:70px; height:70px;" alt="QR">
              <div class="no-print">
                <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block; padding:10px 22px; background:#25D366; color:#fff; border-radius:8px; text-decoration:none; font-weight:700; font-size:12px;">
                  📱 Share via WhatsApp
                </a>
              </div>
            </div>
    ...
    /* ── Category Icons ─────────────────────────────
       Ported directly from src/medicineUtils.js (getMedicineEmoji) so the
       POS shows the exact same icons as the customer-facing site, instead
       of the generic 📦 it was falling back to for every item.
       A real uploaded/URL image (product.image) always wins over this. */
